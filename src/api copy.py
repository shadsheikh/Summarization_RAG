...
LEGAL_SUMMARY_PROMPT = PromptTemplate(
    template="""
    You are a legal assistant. Summarize the following court judgment in clear, structured language:
    - Case background
    - Key issues
    - Court's reasoning
    - Final decision
...
sum_model = pipeline("summarization", model="allenai/led-base-16384")
...
async def upload(file: UploadFile = File(...)):
    ...
    for doc in docs:
        doc.metadata.update({"user_id": user_id, "filename": file.filename})
